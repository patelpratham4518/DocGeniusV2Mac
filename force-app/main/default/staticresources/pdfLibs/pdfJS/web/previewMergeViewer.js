...
var DEFAULT_URL = 'Generatingpreview.pdf';
...
function scrollIntoView(element, spot, skipOverflowHiddenElements) {
  ...
  if (!parent) {
    // composer change, don't show because don't care: console.error('offsetParent is not set -- cannot scroll');
    return;
  }
  ...
  renderPreviewDocumentTitleAndIcon(parent, offsetY);
}
...
function webViewerInitialized() {
  ...
  if (queryString.split('=')[1]) {
    let locale = queryString.split('=')[1].trim();
    if (locale === 'es') { // locale es is not supported
      locale = 'es-ES';
    }
